...
<CodeBlock lang={`shell`} code={`curl -X POST --location 'https://be-demo-ekyc.namtp.id.vn/api/v1/external/workflows/run' \
...
<CodeBlock lang={`shell`} code={`curl -X POST 'https://be-demo-ekyc.namtp.id.vn/api/v1/external/workflows/[WORKFLOW RUNTIME ID HERE]/send-event' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer secret' \
--data '{"name": "start"}'`}/>
